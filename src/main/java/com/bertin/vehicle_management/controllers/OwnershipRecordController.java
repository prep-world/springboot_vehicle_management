...
public class OwnershipRecordController {
    ...
    public ResponseEntity<ApiResponse<String>> transferOwnership(@Valid @RequestBody TransferVehicleDTO dto) {
        try {
            ...
            return ApiResponse.success("Successful.", HttpStatus.CREATED,
                    "Vehicle ownership transferred successfully.");
        } catch (Exception e) {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<List<VehicleOwnershipResponseDTO>>> getOwnershipHistoryByChassis(
            @PathVariable String chassisNumber) {
        try {
            List<VehicleOwnershipResponseDTO> response = ownershipRecordService
                    .getOwnershipHistoryByChassis(chassisNumber);
            return ApiResponse.success("Retrieved ownership records successfully", HttpStatus.OK, response);
        } catch (Exception e) {
            return ApiResponse.fail("Failed to get ownership history by chassis number", HttpStatus.BAD_REQUEST,
                    e.getMessage());
        }
    ...
    @PreAuthorize("hasRole('ADMIN')")
    public ResponseEntity<ApiResponse<List<VehicleOwnershipResponseDTO>>> getOwnershipHistoryByPlate(
            @PathVariable String plateNumber) {
        try {
            List<VehicleOwnershipResponseDTO> response = ownershipRecordService.getOwnershipHistoryByPlate(plateNumber);
            return ApiResponse.success("Retrieved ownership records successfully", HttpStatus.OK, response);
        } catch (Exception e) {
            return ApiResponse.fail("Failed to get ownership history by plate number", HttpStatus.BAD_REQUEST,
                    e.getMessage());
        }
    }
